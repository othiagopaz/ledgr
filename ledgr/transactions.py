"""
Transaction business logic — CRUD operations and models.

Mutations use ``FavaLedger.file`` — never raw ``open()``.
See AGENTS.md §9.
"""
...
from ledgr.serializers import serialize_transaction
...
def _build_bc_postings(postings: list[PostingIn]) -> list[data.Posting]:
    ...
    for p in postings:
        ...
        if p.cost is not None and p.cost_currency:
            cost = CostSpec(p.cost, None, p.cost_currency, None, None, False)
...
def _find_entry_by_lineno(
    entries: Sequence[Directive], lineno: int
...
def add_transaction(
    ...
    try:
        ledger.file.insert_entries([cast(Directive, txn)])
        ledger.load_file()
...
def edit_transaction(
    ...
    try:
        directive = cast(Directive, entry)
        entry_hash_val = hash_entry(directive)
        _, entry_sha = get_entry_slice(directive)
