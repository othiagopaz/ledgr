"""
Report business logic — Income Statement, Balance Sheet, and time series.

Uses ``clamp_opt`` / ``cap_opt`` for correct accounting as required by
AGENTS.md §8.  Delegates to ``ledgr.serializers`` for type conversion.
"""
...
from beancount.core.data import Directive as BcDirective
from beancount.ops import summarize
from fava.core import FavaLedger

from ledgr.cashflow import date_to_period
...
def income_expense_series(
    ...
    for txn in txns:
        ...
        for p in txn.postings:
            units = p.units
            if units is None or units.number is None or units.currency != oc:
                continue
            acct_type = p.account.split(":")[0]
            if acct_type == "Income":
                buckets[period]["income"] += -units.number
            elif acct_type == "Expenses":
                buckets[period]["expenses"] += units.number
...
def account_balance_series(
    ...
    for txn in txns:
        for p in txn.postings:
            units = p.units
            if p.account == account and units is not None and units.number is not None:
                running += units.number
        period = date_to_period(txn.date, interval)
...
def income_statement(
    ...
    entries_list: list[BcDirective] = list(ledger.all_entries)  # type: ignore[arg-type]
    clamped, _ = summarize.clamp_opt(entries_list, begin, end, ledger.options)
...
def balance_sheet(
    ...
    """
    entries_list: list[BcDirective] = list(ledger.all_entries)  # type: ignore[arg-type]
    closed = summarize.cap_opt(entries_list, ledger.options)
