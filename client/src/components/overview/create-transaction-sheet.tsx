...
export function CreateTransactionSheet({
  ...
  const { mutate: submitEvent, isSubmitting } = useCreateEvent({
    onSuccess: resetForm,
  });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (
      ...
      toast.error(
        "Preencha os campos obrigatórios (Valor, Conta/Cartão, Vencimento, Descrição, Categoria)."
      );
      return;
    }

    let payload: CreateEventPayload | null = null;
    try {
      const amountInCents = formatCurrencyToCents(value);
      const formattedDueDate = format(date, "yyyy-MM-dd");
      const formattedPaymentDate =
        !isPending && paymentDate
          ? format(paymentDate, "yyyy-MM-dd")
          : undefined;
      const transactionStatus = isPending
        ? TransactionStatus.SCHEDULED
        : TransactionStatus.PAID;
      const ownershipStatus = Ownership.OWN;
      const fixedNegotiatorId = "00000000-0000-0000-0000-000000000000";

      const selectedInstrument = financialInstruments.find(
        (inst) => inst.id === account
      );
      if (!selectedInstrument) {
        throw new Error("Instrumento financeiro selecionado não encontrado.");
      }

      const transactions: TransactionPayload[] = [];
      if (installmentOption === "À vista") {
        const transaction: TransactionPayload = {
          amount: amountInCents,
          installmentNumber: 1,
          dueDate: formattedDueDate,
          competenceDate: formattedDueDate,
          paymentDate: formattedPaymentDate,
          type: initialType,
          status: transactionStatus,
          ownership: ownershipStatus,
        };
        if (selectedInstrument.kind === "ACCOUNT") {
          transaction.accountId = selectedInstrument.id;
        } else if (selectedInstrument.kind === "CREDIT_CARD") {
          transaction.creditCardId = selectedInstrument.id;
        }
        transactions.push(transaction);
      } else {
        toast.info("Lógica de parcelamento ainda não implementada.");
        return;
      }
      ...
      payload = {
        ...
        categoryId: category,
        negotiatorId: fixedNegotiatorId,
        transactions: transactions,
      };
    } catch (error) {
      console.error("Erro ao construir payload:", error);
      toast.error(
        error instanceof Error ? error.message : "Erro ao preparar dados."
      );
      return;
    }
    ...
    <Sheet>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-grow">
          <fieldset disabled={isSubmitting} className="flex flex-col flex-grow">
